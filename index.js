...
const { getMatchTrendData, getTrendLabel } = require('./trends');
...
async function buildPredictions() {
  ...
  for (const [game, gameMatches] of Object.entries(byGame)) {
    ...
    for (const match of gameMatches) {
      ...
      // Fetch Google Trends data for public sentiment
      let trendData = null;
      try {
        trendData = await getMatchTrendData(match.team1, match.team2, match.tournament, game);
        if (trendData && trendData.sentimentNote) {
          console.log(`    📊 Trend data: ${trendData.sentimentNote.substring(0, 80)}...`);
        }
      } catch (e) {
        console.log(`    Trend check failed (non-fatal): ${e.message}`);
      }
      
      const prediction = await predictMatch(match, teamContext, trendData);
      ...
      const trendLabel = getTrendLabel(trendData);
      ...
      allPredictions.push({
        ...match,
        prediction,
        trendData: trendData ? { team1Interest: trendData.team1.interest, team2Interest: trendData.team2.interest, hypeAdvantage: trendData.hypeAdvantage } : null,
        timestamp: Date.now(),
      });
    }
  }
