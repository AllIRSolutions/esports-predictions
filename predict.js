...
const { formatTrendContext } = require('./trends');
...
async function predictMatch(match, teamContext, trendData = null) {
...
${learningContext ? `\n${learningContext}\n` : ''}
${trendData ? formatTrendContext(trendData) : ''}
...
- reasoning: one concise line explaining the pick`;
  ...
  try {
    const text = await callAI('You are an esports match predictor. Respond ONLY with valid JSON.', prompt, { tier: 'predictions', maxTokens: 200, forceLocal: true });
